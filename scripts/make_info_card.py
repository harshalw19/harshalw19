...
ROWS = [
    ("host",),
    ("kv", "Now", "Site Reliability Engineer @ Tenarai"),
    ("kv", "Prev", "DevOps Intern @ Softtronix"),
    # ("kv", "Also", "AWS/GCP • Kubernetes • CI/CD"),
    ("kv", "Edu", "B.Tech E&TC, SBJITMR '25'"),
    ("gap",),
    ("sec", "Stack"),
    ("kv", "Programming", "Python,Javascript"),
    ("kv", "Cloud", "AWS, GCP, Azure"),
    ("kv", "Devops", "Docker, Kubernetes, Jenkins, Terraform"),
    ("kv", "Observ.", "Prometheus, Grafana,AWS Cloudwatch"),
    ("gap",),
    ("sec", "Highlights"),
    ("bul", "2+ yrs in DevOps & cloud automation"),
    ("bul", "5+ Freelancing projects."),
]
...
parts.append(f'<text x="{W/2}" y="{TITLEBAR_H/2 + 4}" fill="{MUTED}" font-size="12" '
             f'text-anchor="middle">harshalw19@github: ~$ neofetch</text>')
...
for i, row in enumerate(ROWS):
    ...
    if kind == "host":
        inner = (f'<text x="{KEY_X}" y="{y:.1f}" font-size="14" font-weight="700">'
                 f'<tspan fill="{GREEN}">harshalw19</tspan><tspan fill="{MUTED}">@</tspan>'
